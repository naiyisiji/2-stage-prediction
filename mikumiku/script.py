...
class Area_anchor(nn.Module):
    ...
    def forward(self,data):
        init_anchor_pts = self.init_anchor_area(data, self.history_steps, self.dim, self.agent2map_pts_max_length,relative_pos=False)
        ...
        new_query_pos = new_query_pos + pos_a[pred_mask.squeeze(),self.history_steps-1,:].unsqueeze(1).repeat(1,self.query_pts_num,1)
...
def min_covering_area_loss(pred_points):
...
save_path = 'C:\\Users\\Lenovo\\OneDrive - City University of Hong Kong - Student\\Desktop\\mikumiku\\mikumiku'
epoch = 10
...
for ii in range(epoch):
    ...
    for data in loader:
        optimizer.zero_grad()
        pred = testqc(data = data.to(device))
